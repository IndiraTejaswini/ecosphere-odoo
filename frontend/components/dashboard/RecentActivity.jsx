...
import { CheckCircle2, AlertTriangle, Cpu, Bookmark } from 'lucide-react';
...
export default function RecentActivity() {
  const actions = [
    ...
    { text: 'Corporate Affairs revised Governance Compliance metrics', desc: 'New operational weights mapped into the persistent system ledger framework[cite: 1].', time: '4h ago', icon: <Bookmark size={16} className="text-[#A10559]" /> },
  ];

  return (
    <div className="bg-white p-8 rounded-3xl border border-slate-200/60 h-full relative overflow-hidden group hover:shadow-2xs transition-all duration-300">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h3 className="text-lg font-black text-[#0f2438] tracking-tight sm:text-xl">Live Operational Log</h3>
      ...
      <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin">
        {actions.map((act, idx) => (
          <div 
            key={idx}
            className="flex items-start gap-5 p-5 rounded-2xl bg-slate-50/40 hover:bg-slate-50 border border-slate-100 hover:border-slate-200/80 transition-all duration-300 transform hover:-translate-y-1 cursor-pointer group/row hover:shadow-3xs"
          >
            <span className="h-11 w-11 rounded-xl bg-white flex items-center justify-center border border-slate-200/40 shadow-3xs group-hover/row:scale-110 group-hover/row:border-slate-300 group-hover/row:shadow-md transition-all shrink-0">
              {act.icon}
            </span>
            <div className="flex-1 min-w-0 space-y-1.5">
              <div className="flex items-center justify-between gap-4">
                <p className="text-sm font-bold text-slate-700 group-hover/row:text-[#0f2438] transition-colors truncate">{act.text}</p>
