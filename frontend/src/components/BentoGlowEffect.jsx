...
const BentoGlowEffect = ({ 
  children, 
  className = '', 
  glowColor = '22, 63, 161', // The exact StackIt corporate blue signature
  enableTilt = true,
  spotlightRadius = 240
}) => {
  ...
  const handleMouseMove = (e) => {
    ...
    // Set variables for radial cursor tracing tracking
    setCoords({ x, y });

    if (enableTilt) {
      const centerX = rect.width / 2;
      const centerY = rect.height / 2;
      // Controlled constraints to prevent excessive tilting distortion
  ...
  return (
    ...
    >
      {/* 1. INTERNAL CELL SPOTLIGHT BACKGROUND MASK LAYER */}
      <div 
      ...
      {/* 2. CHROME ACTIVE BORDER RADIAL TRACING ILLUMINATION (Matches MagicBento specs exactly) */}
      <div 
        className="absolute inset-0 pointer-events-none z-30 rounded-2xl"
        style={{
          padding: '2.5px',
          // background: `radial-gradient(var(--glow-radius) circle at var(--glow-x) var(--glow-y), rgba(var(--glow-color), 0.08), transparent 80%)`,
